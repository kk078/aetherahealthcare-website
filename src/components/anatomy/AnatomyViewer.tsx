...
type Props = { region: RegionId; layer: AtlasLayer; selected: string; onSelect: (id: string) => void };
...
export default function AnatomyViewer(props: Props) {
  ...
  useEffect(() => {
    ...
    const fit = (back = false) => {
      ...
      for (const mesh of meshes) {
        ...
        if (!s || !mesh.visible || (current.current.selected && s.id !== current.current.selected)) continue;
    ...
    const update = (p: Props) => {
      for (const mesh of meshes) {
        ...
        // A named selection is isolated, so small/internal structures are never hidden by surrounding anatomy.
        if (p.selected) mesh.visible = s.id === p.selected;
        mesh.material.color.set(p.selected === s.id ? '#88e1d1' : color(s.name, s.layer));
      }
      const key = `${p.region}/${p.layer}/${p.selected}`;
    ...
    api.current = {
      ...
      motion: active => { controls.autoRotate = active; render(); },
    };
    const stop = () => { controls.autoRotate = false; setRotating(false); render(); };
    ...
    void Promise.all(Object.values(manifest.files).map(async file => {
      ...
      if (!reduced.matches) { controls.autoRotate = true; setRotating(true); }
      render();
    }).catch(fail);
    return () => {
  ...
  return <div className="overflow-hidden rounded-2xl bg-[#0B2545] text-white">
    ...
    <div ref={host} data-testid="anatomy-canvas" className="h-[470px] w-full sm:h-[560px]" />
