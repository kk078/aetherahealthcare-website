...
import RohenMicroDissectionModal from '@/components/ui/RohenMicroDissectionModal';
import {
  ShieldAlert,
  CheckCircle2,
  FileText,
...
export default function Anatomy3DExplorer() {
  ...
  const [dissectionModalOpen, setDissectionModalOpen] = useState<boolean>(false);
  ...
  return (
    <section id="anatomy-3d-explorer" className="py-20 bg-slate-900 text-white relative overflow-hidden border-t border-slate-800">
          ...
          <div className="lg:col-span-6 bg-slate-850 border border-slate-750 rounded-2xl p-5 shadow-2xl space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setDissectionModalOpen(true)}
                  className="px-3 py-1.5 rounded-xl bg-indigo-600/90 hover:bg-indigo-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-600/20 transition-all cursor-pointer border border-indigo-400/30"
                >
                  <BookOpen className="w-3.5 h-3.5 text-indigo-200" />
                  <span>Rohen Dissection Drill-Down</span>
                </button>
            ...
            {activeHotspot && (
              <div className="p-4 rounded-xl bg-gradient-to-br from-slate-800 via-slate-850 to-slate-900 border border-teal-500/50 shadow-2xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="flex flex-col sm:flex-row gap-2 pt-1">
                  <button
                    onClick={() => setDissectionModalOpen(true)}
                    className="flex-1 py-2 px-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-md shadow-indigo-600/20 cursor-pointer"
                  >
                    <BookOpen className="w-3.5 h-3.5" />
                    Rohen Micro-Dissection
                  </button>
                  <button
                    onClick={() => jumpToHotspotCode(activeHotspot.associatedCode)}
                    className="flex-1 py-2 px-3 rounded-lg bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-md shadow-teal-500/20 cursor-pointer"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                    Jump to {activeHotspot.associatedCode}
                  </button>
                </div>
              </div>
            )}
      ...
      {/* Rohen Atlas Photographic Micro-Dissection Drill-Down Modal */}
      <RohenMicroDissectionModal
        systemId={selectedSystemId}
        isOpen={dissectionModalOpen}
        onClose={() => setDissectionModalOpen(false)}
        onJumpToCode={jumpToHotspotCode}
      />
    </section>
  );
}
