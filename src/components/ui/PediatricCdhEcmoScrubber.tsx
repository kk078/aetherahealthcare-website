...
================================================================================
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!(await sendLeadToKiran('pediatric_cdh_ecmo_rcm_audit', payload))) { setIsSubmitting(false); return; }
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {[
                ...
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setCdhTechnique(item.id as typeof cdhTechnique)}
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {[
                ...
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setEcmoMode(item.id as typeof ecmoMode)}
                  className={`text-left p-3.5 rounded-xl border transition-all duration-150 ${
                    ecmoMode === item.id
