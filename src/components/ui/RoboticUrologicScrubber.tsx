...
================================================================================
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!(await sendLeadToKiran('robotic_urologic_rcm_audit', payload))) { setIsSubmitting(false); return; }
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {[
                ...
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setOncologyProcedure(item.id as typeof oncologyProcedure)}
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {[
                  ...
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => setLymphadenectomyScope(item.id as typeof lymphadenectomyScope)}
                    className={`text-left p-3.5 rounded-xl border transition-all duration-150 ${
